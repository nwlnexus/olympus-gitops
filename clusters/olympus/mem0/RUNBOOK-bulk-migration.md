...
1. Flip categorization OFF in `clusters/olympus/mem0/deployment.yaml`:
   ...
   `flux --context compute-hub reconcile kustomization mem0 --with-source`
   Wait for the rollout: `kubectl --context compute-hub -n mem0 rollout status deploy/openmemory`
...
2. Verify categorization is off (a single add fires ONLY /api/embed, no
   ...
   ```
   POD=$(kubectl --context compute-hub -n mem0 get pod -l app=openmemory -o jsonpath='{.items[0].metadata.name}')
   kubectl --context compute-hub -n mem0 port-forward pod/$POD 18765:8765 &
   ...
   kubectl --context compute-hub -n mem0 logs $POD --since=30s | grep -E "api/embed|chat/completions"
   ```
...
4. Flip categorization BACK ON: set `DISABLE_CATEGORIZATION` value `"false"`,
   commit + push, then `flux --context compute-hub reconcile kustomization mem0 --with-source`.
